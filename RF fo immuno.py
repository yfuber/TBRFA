...
colnames=y_data.columns.values.tolist()
'''
il1bindex=[3,30,48,134,166,213,230]
index_list=list(x_data.index)
for j in il1bindex:
    index_list.remove(j)
x_data=x_data.iloc[index_list,:]
y_data=y_data.iloc[index_list,:]
'''
ss=ShuffleSplit(n_splits=10, test_size=0.1,random_state=0)
...
predf=pd.read_excel('experiment materials.xlsx',sheet_name=1).iloc[0:11,0:31]
